...
class CreateCourseExerciseFormComponent(BaseComponent):
    ...
    @allure.step('Click visible create course exercise form at index "{index}"')
    def check_visible(self, index: int, title: str, description: str):
        self.subtitle.check_visible(index=index)
        self.subtitle.check_have_text(title,index=index)

        self.title_input.check_visible(index=index)
        self.title_input.check_have_value(f"#{index + 1} Subtitle",index=index)
